...
public interface VoterRepository extends JpaRepository<Voter, Integer> {
     ...
     */
    @Query("SELECT v FROM Voter AS v WHERE v.meetingParticipant.id = :participantId AND v.topic.id = :topicId")
     ...
     */
    @Query("SELECT COUNT(v) FROM Voter AS v WHERE v.topic.id = :topicId AND v.vote = 'ЗА'")
     ...
     */
    @Query("SELECT COUNT(v) FROM Voter AS v WHERE v.topic.id = :topicId AND v.vote = 'ПРОТИВ'")
